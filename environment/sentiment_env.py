...
class CyclicalLR(Callback):
    """Robust cyclical learning rate scheduler for Keras/TensorFlow.

    This version handles different shapes of optimizer.learning_rate:
    - tf.Variable (use .assign)
    - objects with .assign method (use it)
    - numeric types (set attribute directly)
    - schedules or unsupported types -> print warning (can't mutate)
    """
    ...
    def _set_optimizer_lr(self, opt, value):
        """Try several safe ways to update optimizer learning rate."""
        # Prefer attribute 'lr' (legacy) then 'learning_rate'
        for attr in ("lr", "learning_rate"):
            if hasattr(opt, attr):
                lr_obj = getattr(opt, attr)

                # 1) If it's a tf.Variable or has assign -> use assign()
                if isinstance(lr_obj, (tf.Variable,)) or hasattr(lr_obj, "assign"):
                    try:
                        # Some objects (tf.Variable, Keras backend variables, etc.)
                        lr_obj.assign(float(value))
                        return True
                    except Exception:
                        # fallback to backend.set_value
                        try:
                            tf.keras.backend.set_value(lr_obj, float(value))
                            return True
                        except Exception:
                            pass

                # 2) If it's a plain number, set it directly on the optimizer
                if isinstance(lr_obj, (float, int)):
                    try:
                        setattr(opt, attr, float(value))
                        return True
                    except Exception:
                        pass

                # 3) If it's an object we cannot mutate (e.g. schedule or string),
                # attempt to replace the attribute with a numeric value (may or may not work)
                try:
                    setattr(opt, attr, float(value))
                    return True
                except Exception:
                    # last resort - can't update
                    print(f"[CLR-WARN] Couldn't set optimizer.{attr} (type={type(lr_obj)}).")
                    return False

        # If optimizer has no lr or learning_rate attribute
        print(f"[CLR-WARN] Optimizer {opt} has no 'lr' or 'learning_rate' attribute.")
        return False

    def on_train_begin(self, logs=None):
        opt = getattr(self.model, "optimizer", None)
        if opt is None:
            raise RuntimeError("Model has no optimizer set (model.optimizer is None).")
        success = self._set_optimizer_lr(opt, self.base_lr)
        if not success:
            # helpful debug output so you can see what's up
            print("[CLR-DEBUG] Failed to set base_lr. Optimizer:", opt)
            # inspect the attributes to diagnose
            if hasattr(opt, "learning_rate"):
                print("[CLR-DEBUG] learning_rate type:", type(opt.learning_rate), "value:", getattr(opt, "learning_rate"))
            if hasattr(opt, "lr"):
                print("[CLR-DEBUG] lr type:", type(opt.lr), "value:", getattr(opt, "lr"))
    ...
    def on_batch_end(self, batch, logs=None):
        ...
        if opt is None:
            return
        success = self._set_optimizer_lr(opt, lr)
        if not success:
            # If we fail, don't crash training — just warn once (or each time if you prefer)
            print(f"[CLR-WARN] Could not update optimizer LR to {lr} (optimizer type: {type(opt)}).")
        ...
        if self.prev_reward < 0:
            ...
            if "lr" in action:
                ...
                if abs(new_lr - self.current_hyperparams["lr"]) > 1e-8:
                      ...
                      # Safely assign learning rate depending on its type
                      try:
                          if isinstance(lr_attr, tf.Variable):
                              lr_attr.assign(float(new_lr))
                          elif hasattr(lr_attr, "assign"):
                              lr_attr.assign(float(new_lr))
                          elif isinstance(lr_attr, (float, int)):
                              setattr(self.model.optimizer, "learning_rate", float(new_lr))
                          else:
                              # last resort — force replace with numeric value
                              self.model.optimizer.learning_rate = tf.Variable(float(new_lr))
                          if self.verbose:
                              print(f"[ENV] Learning rate set to {new_lr:.6f}")
                      except Exception as e:
                          print(f"[WARN] Could not safely set learning rate: {e}")
